...
## Claude Code: preferred setup (plugin)

This skill ships inside the `memotron-memory` Claude Code plugin. Installing
the plugin (`/plugin install memotron-memory@<marketplace>` then
`/reload-plugins`) already provides:

- The `memotron_agent_memory` stdio MCP server (bundled `.mcp.json`, resolved
  via the plugin's `bin/memotron-run` shim — no separate `uv tool install`
  step required before first use).
- `SessionStart`, `PreCompact`, `PostCompact`, and `SessionEnd` hooks that
  register Claude Code, load memory, and run due dreams at session/compaction
  boundaries.
- This skill plus `memotron-memory` for `/memotron-memory:status|why|forget|review`.

The first `SessionStart` hook run in a repository idempotently creates that
repository's `.memotron.yaml` (project identity, mode, storage, and
project-memory policy) if it does not already exist. Validate with:

```bash
memotron status
```

The stdio server and hooks share the repository config and a local graph at
...
extraction. To use a LiteLLM proxy, put the key in the process environment or a
gitignored repository `.env`, then configure only non-secret values:
...
1. Confirm the plugin is installed and `/reload-plugins` has run, then check
   `memotron llm status`.
...
5. Run `memotron llm configure`, verify `llm_ready: true`.

Claude Code uses the plugin's bundled stdio MCP entry and needs no background
daemon. Use `memotron-local-platform` only for hosted HTTP/MCP consumers.
...
Configure the mode in `.memotron.yaml`, or set it explicitly when re-running
the initializer manually:
...
The plugin's bundled Claude Code hooks automate:
...
These four events are the only hook points this integration uses for "dreaming"
(running due Formation/Consolidation/Pruning/Coherence jobs) — Claude Code hooks
have no idle or timer trigger, so dreaming rides session and compaction
boundaries rather than a background schedule.

Customize semantic timing and candidates in `.memotron.yaml` under
`agent_guidance`. Do not edit the generated handlers to ingest transcripts or
publish memories automatically.

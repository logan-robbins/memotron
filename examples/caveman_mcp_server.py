...
CAVEMAN_LEDGER_PATH      sqlite file holding BOTH append-only streams -- the claim
                         ...
                         .memotron/caveman.sqlite). The graph itself is in
                         memory and is a bounded compression of these, so this
...
LITELLM_API_KEY          JedAI Gateway virtual key. Resolved at CALL time, so this
                         process starts without one -- and memory_read,
                         memory_ingest and memory_dream then each answer with one
                         line naming this variable. There is no offline mode.
